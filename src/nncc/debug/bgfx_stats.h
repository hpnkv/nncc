...
void ResourceBar(const char* _name,
                 const char* _tooltip,
                 uint32_t _num,
                 uint32_t _max,
                 float _maxWidth,
...
void ShowBgfxStatsWindow(const char* _errorText) {
    char temp[1024];
...

    {
        ...
        if (ImGui::Button(ICON_FA_REDO " Restart")) {
            cmdExec("app restart");
        }
        ...
        ImGui::SameLine();
        s_showStats ^= ImGui::Button(ICON_FA_BARS);
    ...
    if (s_showStats) {
        ImGui::SetNextWindowSize(
            ImVec2(300.0f, 500.0f), ImGuiCond_FirstUseEver
        );

        if (ImGui::Begin(ICON_FA_BARS " Stats", &s_showStats)) {
                        ...
                        if (ImGui::BeginListBox("Encoders",
                            ...
                            while (clipper.Step()) {
                                for (int32_t pos = clipper.DisplayStart; pos < clipper.DisplayEnd; ++pos) {
                                    ...
                                    const float
                                        cpuMs = float((encoderStats.cpuTimeEnd - encoderStats.cpuTimeBegin) * toCpuMs);
