...
#include <nncc/nodes/poly_value.h>

namespace nncc::nodes {

struct Attribute {
    Attribute(nncc::string _name, const ComputeValue& _value = FloatValue{})
        : id(id_counter++), name(std::move(_name)), value(_value) {}
    ...
    void RenderNodes() {
        for (auto [nodes_current, nodes_end] = boost::vertices(*graph_); nodes_current != nodes_end; ++nodes_current) {
            ...
            for (auto& [name, input] : node.inputs_by_name) {
                ...
                if (input.value.Type() == ComputeValueType::Float) {
                    ImGui::DragFloat("##hidelabel", &As<float>(input.value.Edit()), 0.01f);
                }
                ImGui::PopItemWidth();
